...
def cv_draw_landmark(img_ori, pts, box=None, color=GREEN, size=1, is_dense = True):
    img = img_ori.copy()
    n = pts.shape[1]
    if n <= 106:
        for i in range(n):
    ...
    if not is_dense:
        ...
        plot_close = lambda i1, i2: cv2.line(img, (pts[0, i1], pts[1, i1]), (pts[0, i2], pts[1, i2]),(255, 255, 0), 1,4)
        ...
        for ind in range(len(nums) - 1):
            l, r = nums[ind], nums[ind + 1]
            contour_points = [(pts[0,i1],pts[1,i1]) for i1 in range(l,r)]
            for i in range(r-l-1):
                cv2.line(img, (contour_points[i]),(contour_points[i+1]),(255, 255, 0), 1,4)
